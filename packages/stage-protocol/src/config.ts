...
export interface LitSquareStageProjectConfig {
  name: string;
  sourceEntry: string;
  buildDir: string;
  timeline?: string;
...
export function parseLitSquareStageConfig(input: unknown): LitSquareStageProjectConfig {
  if (!isRecord(input)) {
    throw new Error("stage.config.json must be a JSON object.");
  }
...
function parseRenderSettings(input: unknown): RenderSettings {
  ...
  return {
    ...
    ...(input.motionBlur === undefined ? {} : { motionBlur: parseMotionBlurSettings(input.motionBlur) }),
...
function parseMotionBlurSettings(input: unknown): RenderMotionBlurSettings {
  if (!isRecord(input)) {
    throw new Error("motionBlur must be a JSON object.");
  }

  return {
    enabled: requireBoolean(input, "enabled"),
    shutterAngle: requireNumberInRange(input, "shutterAngle", 0, 360),
    sampleCount: requirePositiveInteger(input, "sampleCount")
  };
}
...
function requireNumberInRange(record: Record<string, unknown>, key: string, min: number, max: number): number {
  const value = record[key];
