import type { FrameContext, LitSquareStageSketch } from "./types.js";
  ...
  return {
    ...
    prepareExport() {
      ...
      return enqueue(async () => {
        await ensureReady();
        await sketch.prepareExport?.(currentContext, root);
        return { ...currentContext };
      });
    },
    ...
    finishExport() {
      ...
      return enqueue(async () => {
        await ensureReady();
        await sketch.finishExport?.(currentContext, root);
        return { ...currentContext };
      });
    },
    destroy() {
